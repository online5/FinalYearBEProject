...
def complete_anpr(request):
    image_list=MovementDetection()#Deepak Module
    ...
    if flag == True:
        ...
        if(Resident.objects.filter(Resident_Vehicle_Number=output_string).exists()):
            ...
            #return HttpResponse("Welcome Visitor :)")
        #html = "<html><body>Output String is %s.</body></html>" % output_string
...
def load_once():
    ...
    classes=[]
    with open("web/coco.names", "r") as f:
        classes = [line.strip() for line in f.readlines()]

    net = cv2.dnn.readNet("web/yolov3.weights", "web/yolov3.cfg")
    json_file = open('web/MobileNets_first_character_recognition.json', 'r')
    loaded_model_json = json_file.read()
    json_file.close()
    first_letter_model = model_from_json(loaded_model_json)

    first_letter_model.load_weights("web/first_letter_dataset.h5")
    print("First Letter Model loaded successfully...")

    first_letter_labels = LabelEncoder()
    first_letter_labels.classes_ = np.load('web/license_first_character_classes.npy')
    print("First Letter Labels loaded successfully...")

    json_file = open('web/MobileNets_second_character_recognition.json', 'r')
    loaded_model_json = json_file.read()
    json_file.close()
    second_letter_model = model_from_json(loaded_model_json)

    second_letter_model.load_weights("web/second_letter_dataset.h5")
    print("Second Letter Model loaded successfully...")

    second_letter_labels = LabelEncoder()
    second_letter_labels.classes_ = np.load('web/license_second_character_classes.npy')
    print("Second Letter Labels loaded successfully...")

    json_file = open('web/MobileNets_digit_recognition.json', 'r')
    loaded_model_json = json_file.read()
    json_file.close()
    digit_model = model_from_json(loaded_model_json)

    digit_model.load_weights("web/digits_dataset.h5")
    print("Digit Model loaded successfully...")

    digit_labels = LabelEncoder()
    digit_labels.classes_ = np.load('web/license_digit_classes.npy')
    print("Digit Labels loaded successfully...")

    json_file = open('web/MobileNets_character_recognition.json', 'r')
    loaded_model_json = json_file.read()
    json_file.close()
    model = model_from_json(loaded_model_json)
    model.load_weights("web/License_character_recognition.h5")
    print("Model loaded successfully...")

    labels = LabelEncoder()
    labels.classes_ = np.load('web/license_character_classes.npy')
    print("Labels loaded successfully...")


# Render to Dashboard page login if credentials are valid
